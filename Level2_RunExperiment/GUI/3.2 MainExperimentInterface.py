...
- Timeline visualization for experiment events
...
class AudioPlayer:
    ...
    def prepare_playback(self, text_widget=None, buffer_size=512, test_tones=False):
        ...
        if test_tones:
            # Create a sequence of 5 identical tones to make latency visible
            tone_frequency = 1000  # Hz
            tone_duration = 0.02  # seconds (20ms) - no fade in/out for visual clarity
            tone_amplitude = 0.25  # 25% of maximum volume
            num_tones = 5
            gap_duration = 0.1  # seconds between tones
            
            # Calculate total test signal duration
            test_duration = num_tones * tone_duration + (num_tones - 1) * gap_duration
            test_samples = int(test_duration * device_sr)
            
            # Create empty test signal array
            test_signal = np.zeros(test_samples, dtype=np.float32)
            
            # Generate the sequence of tones
            for i in range(num_tones):
                # Calculate start and end sample for each tone
                start_sample = int(i * (tone_duration + gap_duration) * device_sr)
                end_sample = int(start_sample + tone_duration * device_sr)
                
                # Generate tone time array
                t = np.linspace(0, tone_duration, end_sample - start_sample, endpoint=False)
                
                # Generate the tone (1000 Hz sine wave) - no fade in/out
                tone = tone_amplitude * np.sin(2 * np.pi * tone_frequency * t).astype(np.float32)
                
                # Place tone in test signal
                test_signal[start_sample:end_sample] = tone
                
            # Create a new longer audio array to PREPEND the test tones
            new_length = len(self.audio_data) + test_samples
            new_audio = np.zeros((new_length, 2), dtype=np.float32)
            
            # Place identical test signal on both channels
            new_audio[:test_samples, 0] = test_signal  # Left channel
            new_audio[:test_samples, 1] = test_signal  # Right channel - identical
            ...
            debug_print(f"Prepended test tones: {num_tones} identical 20ms 1000Hz tones on both channels ({test_duration:.2f} seconds total)", 1, text_widget)
...
class BreathingSpaceApp:
    ...
    and mouse recentering before tactile stimuli.
    """
    def __init__(self, root):
        ...
        # Make window fixed in place (non-movable)
        ...
        # Add a close button since we've removed window decorations
        self._create_close_button()
        
        # Load participant info automatically
        self.load_participant_from_metadata()
        
        # Report play button coordinates
        self.root.after(1000, self.report_button_coordinates)
        
        # Debug output
        debug_print("Application started", 0, self.log_text)
    
    def report_button_coordinates(self):
        """Report the exact coordinates of the play button."""
        # Wait for the window to be fully rendered
        self.root.update_idletasks()
        
        # Get the button's position relative to the screen
        button_x = self.play_button.winfo_rootx()
        button_y = self.play_button.winfo_rooty()
        
        # Get the button's width and height
        button_width = self.play_button.winfo_width()
        button_height = self.play_button.winfo_height()
        
        # Calculate the button's borders
        left = button_x
        top = button_y
        right = button_x + button_width
        bottom = button_y + button_height
        
        debug_print("==========================================", 0, self.log_text)
        debug_print("PLAY BUTTON EXACT COORDINATES:", 0, self.log_text)
        debug_print(f"Left border: {left}px", 0, self.log_text)
        debug_print(f"Top border: {top}px", 0, self.log_text)
        debug_print(f"Right border: {right}px", 0, self.log_text)
        debug_print(f"Bottom border: {bottom}px", 0, self.log_text)
        debug_print(f"Width: {button_width}px, Height: {button_height}px", 0, self.log_text)
        debug_print("==========================================", 0, self.log_text)
    
    def _create_close_button(self):
        """Create a more visible close button since we've removed the window decorations"""
        exit_font = font.Font(family="Arial", size=12, weight="bold")
        ...
        # Add style to make the button more visible
        style = ttk.Style()
        style.configure("Exit.TButton", 
                       foreground="white", 
                       background="red",
                       font=exit_font)
        close_button.configure(style="Exit.TButton")
    ...
    def start_experiment(self):
        """Start the experiment by playing audio."""
        ...
        try:
            ...
            buffer_size = 128  # Default to small buffer size
    ...
    def _start_experiment_thread(self, buffer_size, start_time):
        ...
        try:
            # Update status for loading file
    ...
    def on_closing(self):
        ...
        try:
            ...
            debug_print(f"Error during cleanup: {e}", 0, self.log_text)
