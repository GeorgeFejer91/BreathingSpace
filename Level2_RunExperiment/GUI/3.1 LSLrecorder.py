...
- Positions itself in the left side of the screen with bottom edge at screen bottom
...
TONE_DURATION = 0.02        # seconds - reduced to 20ms
...
class ToneGenerator:
    ...
    def _generate_tone(self):
        """Generate the tone signal without fade in/out"""
...
class CombinedLSLApp:
    ...
    def setup_styles(self):
        ...
        # Create a style for the flash effect on the refresh button
        style.configure("Flash.TButton", background="red", foreground="white", font=("Arial", 10, "bold"))
        
        # Create styles for success/failure indicators
    ...
    def position_window(self):
        """Position window on the left side of the screen with bottom edge at screen bottom"""
        # Get screen dimensions
        screen_width = self.root.winfo_screenwidth()
        screen_height = self.root.winfo_screenheight()
        
        # Set window to take up full left side with 75% height
        window_width = int(screen_width * 0.5)  # 50% of screen width
        window_height = int(screen_height * 0.75)  # 75% of screen height
        
        # Position at left side with bottom at screen bottom
        x_position = 0  # Left side of screen
        y_position = screen_height - window_height  # Bottom aligned
    ...
    def _build_overview_tab(self):
        ...
        # Verification status indicator (large checkmark or X)
        verification_indicator_frame = ttk.LabelFrame(overview_frame, text="Stream Verification Status", padding=10)
        verification_indicator_frame.pack(fill=tk.X, pady=5)
        
        # Create a canvas for the verification indicator
        self.verification_canvas = tk.Canvas(verification_indicator_frame, width=400, height=100, bg="white")
        self.verification_canvas.pack(fill=tk.X, pady=5)
        
        # Create initial "waiting" indicator
        self.verification_canvas.create_text(
            200, 30, 
        ...
        self.verification_message = self.verification_canvas.create_text(
            200, 70, 
    ...
    def display_verification_result(self, success, message):
        """Display verification result with clear visual feedback"""
        # Clear canvas
        self.verification_canvas.delete("all")
        
        # Switch to the Overview tab to make sure the user sees the result
        self.notebook.select(0)
        
        if success:
            # Draw a large green checkmark
            self.verification_canvas.create_text(
                200, 50, 
                text="✓", 
                font=("Arial", 60, "bold"), 
            ...
            self.verification_canvas.create_text(
                200, 90, 
                text="STREAMS VERIFIED SUCCESSFULLY", 
                font=("Arial", 12, "bold"), 
                fill="green"
            )
        else:
            ...
            # Add more detailed message with instructions
            self.verification_canvas.create_text(
                200, 110, 
                text=message, 
                font=("Arial", 10), 
            ...
            self.verification_canvas.create_text(
                200, 130, 
                text="Please check hardware connections and press 'Refresh'", 
                font=("Arial", 9, "italic"), 
                fill="blue"
            )
            
            # Make the refresh button flash to draw attention
            self.flash_refresh_button()
    
    def flash_refresh_button(self, count=0):
        """Make the refresh button flash to draw attention"""
        if count > 10:  # Stop after 5 flashes (10 color changes)
            self.refresh_button.configure(style="TButton")
            return
            
        if count % 2 == 0:
            # Create a highlighted button style
            style = ttk.Style()
            style.configure("Flash.TButton", background="red", foreground="white")
            self.refresh_button.configure(style="Flash.TButton")
        else:
            self.refresh_button.configure(style="TButton")
            
        # Schedule next flash
        self.root.after(500, lambda: self.flash_refresh_button(count + 1))
    ...
    def verify_streams(self):
        ...
        # Update verification display to show we're verifying
        self.verification_canvas.delete("all")
        self.verification_canvas.create_text(
            200, 50, 
            text="⟳", 
        ...
        try:
            ...
            if not all_streams:
                ...
                # Display failure with message
                self.display_verification_result(False, "No LSL streams found. Is the LSL network active?")
                return False
            ...
            if mouse_found and audio_found:
                ...
                # Display success
                self.display_verification_result(True, "")
                return True
            else:
                missing = []
                if not mouse_found:
                ...
                # Display failure with specific message
                failure_msg = f"Unable to find {', '.join(missing)} stream(s). Please check audio connections."
                self.display_verification_result(False, failure_msg)
                return False
            ...
            self.display_verification_result(False, f"Error during verification: {str(e)}")
            return False
    ...
    def _click(self, idx):
        ...
        # Check if this is a play button click (using exact coordinates)
        is_play_button = (1461 <= x <= 1535) and (77 <= y <= 107)
        
        # Create event name to include in LSL stream
        button_name = "left" if idx == 0 else "right" if idx == 1 else "middle"
        
        if is_play_button:
            event_name = f"PLAY_BUTTON_{button_name}"
            # Stream the click data to LSL with additional metadata
            # We'll keep the existing 3-value format for compatibility
            self.mouse_stream.push_sample([t, float(x), float(y)])
            
            # Log prominently
            self.log(f"!!! {event_name} CLICK DETECTED @ {t:.3f}s ({x},{y}) !!!")
            self.status_var.set(f"PLAY BUTTON CLICKED at {t:.3f}s")
            
            # Update event stream for play button event
            desc = self.mouse_stream.info().desc()
            desc.append_child_value("last_event", event_name)
            desc.append_child_value("last_play_click_time", str(t))
        else:
            event_name = f"regular_{button_name}_click"
            # Stream regular click
            self.mouse_stream.push_sample([t, float(x), float(y)])
            
            # Standard log
            self.log(f"Click: {button_name} @ {t:.3f}s ({x},{y}) - {event_name}")
    ...
    def auto_start(self):
        ...
        if self.participant_id in ["Unknown", "Error"]:
            ...
            # Update verification display for error
            self.verification_canvas.delete("all")
            self.verification_canvas.create_text(
                200, 50, 
                text="!", 
            ...
            self.verification_canvas.create_text(
                200, 90, 
                text="NO VALID PARTICIPANT ID", 
                font=("Arial", 12, "bold"), 
                fill="orange"
            )
            self.verification_canvas.create_text(
                200, 110, 
                text="Please run the Participant Logger first", 
                font=("Arial", 10), 
                fill="black"
            )
            return
        ...
        # Update verification display to show we're starting
        self.verification_canvas.delete("all")
        self.verification_canvas.create_text(
            200, 50, 
            text="⟳", 
        ...
        if self.scan_streams():
            ...
            # If we reach here, we've fully succeeded
            if self.recording:
                # Update verification display to show full success
                self.verification_canvas.delete("all")
                self.verification_canvas.create_text(
                    200, 40, 
                    text="✓", 
                ...
                self.verification_canvas.create_text(
                    200, 110, 
                    text=f"Recording all streams for {self.participant_id}", 
                    font=("Arial", 10), 
                    fill="black"
                )
        else:
    ...
    def refresh_and_restart(self):
        ...
        self.participant_id, self.participant_metadata = get_participant_info()
        self.participant_var.set(f"ID: {self.participant_id}")
